                    ...
                    <div class="ibox-content">
                        <form action="{{ route('role.update', $role) }}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="rolname" name="rolname" value="{{ old('rolname',$role->name) }}">
                            </div>
                            <div class="row">
                                                ...
                                                @foreach($permissions as $permission=>$son)
                                                    <div class="input-group" style="margin-top: 5px">
                                                        ...
                                                        <span class="input-group-btn">
                                                            <button class="btn btn-white btnRol @if($role->hasPermission($permission)) active @endif" data-permission="{{ $permission }}" type="button" data-toggle="button">
                                                                <a class="fa fa-check" style="text-decoration: none"></a>
                                                            </button>
                                                        </span>
                                                        <input type="checkbox" id="check_{{ $permission }}" value="{{ $permission }}" name="check_permission[]" @if($role->hasPermission($permission)) checked @endif style="display: none">
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
